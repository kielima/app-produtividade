import { parseWikilinks, normalizeNoteName } from './grafosWikilink';
import { buildNameIndex, resolveWikilinkTarget } from './grafosBacklinks';
import { findParentFolderId, type VaultState } from './grafosTreeState';
...
// testável isoladamente (grafosGraph.test.ts). Opera direto sobre
// VaultState (a mesma estrutura chaveada por id da Fase 1), sem precisar de
// nenhum estado adicional: a árvore (src/views/GrafosView.tsx) e o grafo
// (src/components/GrafosGraphView.tsx) são só duas visualizações da mesma
// fonte de verdade.
...
// que permite ao grafo (GrafosGraphView.tsx) decidir se um nó tipo 'file'
