...
import { findParentFolderId } from '../lib/grafosTreeState';
...
// grafosColors.ts (compartilhados com o sistema solar, que tem sua
// própria paleta/shape — ver GrafosSolarSystemView.tsx).
type GraphColors = {
  folder: string;
  note: string;
  file: string;
...
export function GrafosGraphView({
  vault,
  onEditNote,
  onNodeDeleted,
}: {
  ...
  // Chamado depois de excluir com sucesso — a árvore (GrafosView.tsx) usa
  ...
  // HTML tem sua própria janela (ver GrafosHtmlViewerDialog) em vez do
  ...
  const nodeCanvasObject = useCallback(
    (node: GNode, ctx: CanvasRenderingContext2D, globalScale: number) => {
      ...
      // (`GrafosNoteGraphCard`) ser posicionado em cima dela logo em
  ...
  return (
    <div className="grafos-graph-view" ref={containerRef}>
      ...
      {ghostWarning && <p className="error grafos-status-line grafos-graph-warning">{ghostWarning}</p>}
      ...
      {actionStatus && (
        <p className="error grafos-status-line grafos-graph-warning">{actionStatus}</p>
      )}
