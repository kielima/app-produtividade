...
// de `ObsidianNoteGraphCard`, ver ObsidianGraphView.tsx) — sem edição, só
...
export const ObsidianFilePreviewCard = forwardRef<
  HTMLDivElement,
  { fileId: string; mimeType: string; readFileBytes: (fileId: string) => Promise<ArrayBuffer> }
>(function ObsidianFilePreviewCard({ fileId, mimeType, readFileBytes }, ref) {
  ...
  return (
    <div ref={ref} className="obsidian-note-graph-card obsidian-file-preview-card" role="group" aria-label="Preview de arquivo">
      {status === 'loading' && <p className="muted">Carregando…</p>}
      {status === 'error' && <p className="error">{error}</p>}
      {status === 'loaded' && bytes && (isImage ? <ObsidianImagePreview bytes={bytes} mimeType={mimeType} /> : <ObsidianPdfPagePreview bytes={bytes} />)}
    </div>
  );
});

function ObsidianImagePreview({ bytes, mimeType }: { bytes: ArrayBuffer; mimeType: string }) {
...
function ObsidianPdfPagePreview({ bytes }: { bytes: ArrayBuffer }) {
